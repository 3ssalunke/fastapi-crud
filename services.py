...
def get_current_user(db: _orm.Session=_fastapi.Depends(get_db), token: str=_fastapi.Depends(oauth2Schema)):
    try:
...
def create_lead(user: _schemas.User, db: _orm.Session, lead: _schemas.LeadCreate):
...
def get_leads(user: _schemas.User, db: _orm.Session):
    leads = db.query(_models.Lead).filter_by(owner_id=user.id)

    return list(map(_schemas.Lead.from_orm, leads))


def _lead_selector(lead_id: int, user: _schemas.User, db: _orm.Session):
...
def get_lead(lead_id: int, user: _schemas.User, db: _orm.Session):
    lead = _lead_selector(lead_id, user, db)

    return _schemas.Lead.from_orm(lead)


def delete_lead(lead_id: int, user: _schemas.User, db: _orm.Session):
    lead = _lead_selector(lead_id, user, db)
...
def update_lead(lead_id: int, lead: _schemas.LeadCreate, user: _schemas.User, db: _orm.Session):
    lead_db = _lead_selector(lead_id, user, db)
